"""Invoice number value object for finance domain."""
...
class InvoiceNumber(ValueObject):
    """Strongly typed invoice number."""

    value: str

    def __post_init__(self) -> None:
        if not isinstance(self.value, str) or not self.value.strip():
            raise InvalidInvoiceReferenceError("invoice_number must be a non-empty string")
