...
from mfm.application.certificates.create_certificate import to_issuer_reference
...
class RenewCertificateUseCase:
    """Renew certificate through domain renewal operation."""
    ...
    def execute(self, request: RenewCertificateRequest) -> RenewCertificateResponse:
        ...
        try:
            with self._unit_of_work as uow:
                ...
                source_certificate = repository.get_by_id(request.source_certificate_id)
                if source_certificate is None:
                    raise BusinessRuleViolation(
                        f"Certificate {request.source_certificate_id} does not exist"
                    )

                renewed_certificate = source_certificate.renew(
                ...
                repository.add(renewed_certificate)
                uow.commit()
            ...
            raise RepositoryException("Renew certificate failed") from exc

        return RenewCertificateResponse(
            source_certificate_id=source_certificate.id.value,
            renewed_certificate=to_certificate_response(renewed_certificate),
        )
